...
a) There is a strong positive linear correlation between shoulder girth and height.
b) The relationship would stay the same, but the x scale would be different.  I have plotted it below for reference.

```{r}
library(tidyverse)
bdimstmp <- bdims %>% select(hgt, sho_gi)
bdimstmp <- bdimstmp %>% mutate(sho_gi_inches = sho_gi / 2.54)

# plot height vs. shoulder girth in inches------------------------------------
par(mar = c(3.8, 3.8, 0.5, 0.5), las = 1, mgp = c(2.7, 0.7, 0), 
    cex.lab = 1.25, cex.axis = 1.25)
plot(bdimstmp$hgt ~ bdimstmp$sho_gi_inches, 
...
a) \[
  \hat{y} = 105.9651 + 0.60797 \times shoulder\_girth
\]
b) The slope is 0.60797, and the intercept is 105.9651.  This indicates a positive correlation.
c) 44.89%
d) 166.7626
e) The residual is -6.7626 in this case.  Residual is the difference between the observed value and what was predicted (i.e. the error).
f) No because it falls outside of the scope of the observed data that we have used to build the model.
...
a) \[
  \hat{y} = -0.357+ 4.034 \times body\_weight
\]
...
a) To find the slope we just need to solve for b (since we have x and y from the means).  Per the below calculation, the slope (i.e. 'b') is 0.1325028.
b) Yes, because the slope as calculated above is positive.
c) The conditions (from pg. 318) are:
