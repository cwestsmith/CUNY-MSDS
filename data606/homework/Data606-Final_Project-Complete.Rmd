...
To further check normality histograms were created for each group:
...
nonparents_mean <- mean(parents_df$life_satisfaction)
parents_mean <- mean(nonparents_df$life_satisfaction)
...
cat("The mean level of life satisfaction for non-parents is", round(nonparents_mean,2),"\n")
cat("The mean level of life satisfaction for parents is", round(parents_mean,2))
...
cat("The 95% Confidence Interval is between", round(interval_low, 3), "and", round(interval_high, 3))
...
cat("The p-value is", round(pvalue_manual,3), "so we", result, "the null hypothesis")
```
...
# Searching For (Better) Truth

## Additional analysis

With the disappointing results of the t-test, I performed some more analysis to see if perhaps there was a difference in regions.  To do this, I had to find a way to create a column that associated a global region with each country code.
...
plot_grid(plot_avg,
          plot_region,
          labels = c('By # of Children','By Region'),
          label_x = 0.2,
          ncol = 2)



# Interesting - it seems satisfaction is quite a bit better after 13 children - 14 to 15 seems to be 'ideal'
nonparents_df %>% 
  group_by(region) %>% 
  summarise(avg_life_satisfaction = mean(life_satisfaction)) %>% 
  arrange(desc(avg_life_satisfaction))
...
ggplot(data=parents_df, aes(life_satisfaction)) + geom_histogram(binwidth = 1) + ggtitle("Histogram: Life satisfaction for parents w/ 14 or more children.  Normal?")
...
In conclusion, after performing a t-test the null hypothesis can be rejected in favor of the alternative hypothesis, which is that there is a difference in the levels of life satisfaction between non-parents and parents.  Somewhat surprisingly it seems the difference is negative, albeit small - with a 95% confidence interval of between -0.225 and -0.161.  However, hope for parents is not lost.

The null hypothesis in fact holds true when focused on parents with 14 or more children.  In addition, if a one-sided t-test is used instead of a two-sided and the null hypothesis is correspondingly updated to "non-parents do not have a lower level of life satisfaction" then the null hypothesis can in fact be rejected in favor of the alternate hypothesis.  The real conclusion is therefore that parents are happier than non-parents, as long as they have at least 14 children.

On a more serious note, this project has been an interesting experiment with data and has helped to illustrate the pitfalls of trying to use t-tests with ordinal variables as well as why one-tailed tests can sometimes be controversial.  When I did not like the results of my experiment I simply needed to change the rules of the game.
