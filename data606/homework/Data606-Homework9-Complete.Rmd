...
(b) Babies born to smokers tend to be 8.94 ounces less on average.  To calculate the weight for those whose mother smoked, the equation would be: Weight = 123.05 - 8.94, i.e. 114.11.  The weight of babies born to non-smokers would be 123.05 (because the -8.94 would be multiplied by 0).
(c) Yes it seems so given the p-value of 0 (assuming .05 is the threshold for significance).
...
(b) Non-aboriginal ethnicity reduces the predicted # of days by 9.11, males increase the # by 3.10, and slow learners increase the # by 2.15.
(c) The predicted value is 23.18 versus the actual value of 2, so the residual is 21.18.
(d) R squared = .089, and adjusted R squared = .070.

```{r}
# Math for part c above
predicted_value <- 18.93 - 9.11 * 0 + 3.10 * 1 + 2.15 - 1
actual_value <- 2
predicted_value - actual_value

# Match for part d above
# Note to self: see pg. 349
var_res <- 240.57
var_absdays <- 264.17
rsquared <- 1 - (var_res / var_absdays)
adj_rsquared <- 1 - (var_res / var_absdays) * ((146 - 1) / (146 - 3 - 1))
...
The 'learner status' variable should be removed first as it has the highest p-value.
...
(a) It appears that over 60 degrees or so it is much less common for o rings to fail than when the temperature is below 60 degrees.
(b) The y intercept is 11.663, and the slope change due to temperature is -0.2162 per degree, indicating that as the temperature increases, the likelihood of failure also decreases.  The p-value for temperature is 0.00, indicating that the associated change is likely not due to chance and thus it is statistically significant.
(c) Likelihood = 11.6630 - .2162 * temperature.
(d) Yes, it seems to given that the p-value is so low.
...
(a) 65.4%, 55.1%, 44.3%.  See code block below for calculations.
(b) See chart below.
(c) No specific concerns or assumptions other than assuming that the two logistic regression conditions are true, in particular that each outcome is independent of the other outcomes (which seems to be satisfied here), and that each predictor is linearly related to logit(p sub i) if all other predictors are held constant.

```{r warning=FALSE}
# Calculations for part a
equation51 <- 11.6630 - .2162 * 51
equation53 <- 11.6630 - .2162 * 53
equation55 <- 11.6630 - .2162 * 55 

answer51 <- round(exp(equation51)/(1+exp(equation51)), 3)
answer53 <- round(exp(equation53)/(1+exp(equation53)), 3)
answer55 <- round(exp(equation55)/(1+exp(equation55)), 3)

# Chart for part b (adapted from code above)
library(ggplot2)
plotpoints <- data.frame(c(57, 59, 61, 63, 65, 67, 69, 71, 51, 53, 55), c(.341, .251, .179, .124, .084, .056, .037, .024, .654, .551, .443))
colnames(plotpoints) <- c("temp", "probability")
ggplot(plotpoints, aes(x=temp, y=probability)) + 
  geom_point() +
